...
fn align_to_four_bytes(n: usize) -> u32 {
    (n as u32 + 3) % 4
}

fn as_u8_slice<T: Sized>(p: &[T]) -> &[u8] {
    // size_of T must be a multiple of 4 bytes
    assert_eq!(size_of::<T>() % 4, 0);
    let len = p.len();
    let p = (p as *const [T]) as *const u8;
    unsafe { from_raw_parts(p, len * size_of::<T>()) }
}

fn export_glb<V: Vtx>(filename: &str, vertices: &[V]) {
    let min = vertices
        .iter()
        .map(|v| v.pos())
    ...
    let root_len = align_to_four_bytes(root_json.len());
    let glb = Glb {
        header: Header {
            magic: *b"glTF",
            version: 2,
